...
private static final int portAuth = 6380;
  ...
  public void setUp() throws Exception {
    ...
    dest.auth("0b0b363813be884a827682f3657f770a");
    dest.flushAll();
    dest.select(db);
  ...
  public void migrate() {
    jedis.set("foo", "bar");
    assertEquals("OK", jedis.migrate(host, port, db, timeout, new MigrateParams().auth("0b0b363813be884a827682f3657f770a"), "foo"));
  ...
  public void migrateEmptyParams() {
    ...
    jedis.set(bfoo, bbar);
    assertEquals("OK", jedis.migrate(host, port, db, timeout, new MigrateParams().auth("0b0b363813be884a827682f3657f770a"), bfoo));
    assertArrayEquals(bbar, dest.get(bfoo));
    assertNull(jedis.get(bfoo));
  }
  ...
  public void migrateCopy() {
    jedis.set("foo", "bar");
    MigrateParams migrateParams = new MigrateParams();
    migrateParams.auth("0b0b363813be884a827682f3657f770a");
    migrateParams.copy();
    assertEquals("OK", jedis.migrate(host, port, db, timeout, migrateParams, "foo"));
    assertEquals("bar", dest.get("foo"));
    assertEquals("bar", jedis.get("foo"));

    jedis.set(bfoo, bbar);
    assertEquals("OK", jedis.migrate(host, port, db, timeout, migrateParams, bfoo));
    assertArrayEquals(bbar, dest.get(bfoo));
  ...
  public void migrateMulti() {
    ...
    assertEquals("OK", jedis.migrate(host, port, db, timeout, new MigrateParams().auth("0b0b363813be884a827682f3657f770a"), "foo1", "foo2", "foo3"));
    ...
    assertEquals("OK", jedis.migrate(host, port, db, timeout, new MigrateParams().auth("0b0b363813be884a827682f3657f770a"), bfoo1, bfoo2, bfoo3));
  ...
  public void migrateConflict() {
    ...
    dest.set("foo2", "bar");
    try {
      jedis.migrate(host, port, db, timeout, new MigrateParams().auth("0b0b363813be884a827682f3657f770a"), "foo1", "foo2", "foo3");
    ...
    dest.set(bfoo2, bbar);
    try {
      jedis.migrate(host, port, db, timeout, new MigrateParams().auth("0b0b363813be884a827682f3657f770a"), bfoo1, bfoo2, bfoo3);
      fail("Should get BUSYKEY error");
    } catch(JedisDataException jde) {
      assertTrue(jde.getMessage().contains("BUSYKEY"));
    }
